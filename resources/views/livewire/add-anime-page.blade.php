        ...
        <flux:card class="space-y-6">
            ...
            <div class="space-y-3">
                ...
                @if(count($nameSuggestions) > 0)
                    <flux:radio.group wire:model.live="selectedNameOption" variant="cards" class="max-sm:flex-col">
                        @foreach($nameSuggestions as $suggestion)
                            <flux:radio value="{{ $suggestion['name'] }}">
                                <flux:radio.indicator />
                                <div class="flex flex-1 items-center justify-between gap-2">
                                    <flux:heading size="sm" class="leading-4">{{ $suggestion['name'] }}</flux:heading>
                        ...
                        <flux:radio value="custom">
                            <flux:radio.indicator />
                            <flux:heading size="sm" class="leading-4">{{ __('Custom name') }}</flux:heading>
                        </flux:radio>
                    </flux:radio.group>
                @endif
            ...
            @if(count($seasons) > 0)
                <div class="space-y-3">
                    ...
                    <flux:checkbox.group wire:model="selectedSeasons" variant="cards" class="max-sm:flex-col">
                        @foreach($seasons as $season)
                            <flux:checkbox value="{{ $season['number'] }}">
                                <flux:checkbox.indicator />
                                <div class="flex flex-1 items-center justify-between gap-2">
                                    <div>
                                        <flux:heading size="sm" class="leading-4">{{ __('Season') }} {{ $season['number'] }}</flux:heading>
                                        <flux:text size="sm" class="text-zinc-500 dark:text-zinc-400">
        ...
        <flux:card class="overflow-hidden p-0">
            ...
            <div class="divide-y divide-zinc-200 dark:divide-zinc-700">
                @foreach($results as $result)
                    <div class="flex flex-col gap-3 p-4 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50 sm:flex-row sm:items-start">
